...
class EnvironmentRecyclerViewAdapter(
    private val tasks: MutableList<EnvironmentResponse> = mutableListOf(),
    private val clickListener: (EnvironmentResponse) -> Unit
) : RecyclerView.Adapter<EnvironmentRecyclerViewAdapter.MyViewHolder>() {

    fun updateTasks(newTasks: List<EnvironmentResponse>) {
        tasks.clear()
